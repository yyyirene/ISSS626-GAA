...
```{r}
ggplot() +
  geom_sf(data = selected_roads, color = "black", size = 0.5) +
...
### 3.3 Convert the road data to LINESTRING
...
```{r}
road_i_t_filtered <- road_i_lines %>%
  filter(highway != "secondary")
```

```{r}
road_b <- st_intersection(Bangkok,road_i_t_filtered)
...
### 3.4 **Preparing the lixels objects**
...
### 3.4.1 **Generating line centre points**

```{r}
samples <- lines_center(lixels) 
```

### **3.4.2 Performing NKDE**
...
### **3.4.3** Visualising NKDE

```{r}
tm_shape(Bangkok) + 
  tm_borders(col = "black", lwd = 1) +  
  tm_text("ADM1_EN", size = 1, col = "red", shadow = TRUE) +
tm_shape(lixels) + 
...
#### **3.4.3.2** Visualising **the target areas**

```{r}
tmap_mode("plot")
tm_shape(Bangkok_b) + 
  tm_borders(col = "black", lwd = 1) +  
  tm_text("ADM2_EN", size = 0.4, col = "blue", shadow = TRUE) +  
tm_shape(lixels) + 
  tm_dots(col = "density", style = "kmeans", n = 8, palette = "viridis", size = 0.02) +  
  tm_layout(legend.outside = TRUE)  
```

we found that both of them happen in the boundary between "Lat Krabang" and Prawet" as well as the area near the airport.

#### **3.4.3.3** Computing K-fucntion Estimate

```{r}
Lat_Pra_b <- admin_level_2_filtered_32647 %>%
  filter(ADM2_EN %in% c("Lat Krabang", "Prawet"))
```

```{r}
acci_ppp <- as.ppp(acci)

Lat_Pra_b_owin <- as.owin(Lat_Pra_b)

acci_tm_ppp <- acci_ppp[Lat_Pra_b_owin]
...
K_lp.csr <- envelope(acci_tm_ppp, Kest, nsim = 20, rank = 1, glocal=TRUE)
...
#### **3.4.4** Finding the highest density
...
```{r}
lixels_df <- as.data.frame(lixels)

lixels_df$density_category <- cut(lixels_df$density, 
                                  breaks = c(-Inf, 0.2, 0.4, 0.6, Inf),
                                  labels = c("0-0.2", "0.2-0.4", "0.4-0.6", ">0.6"))

density_segments_list <- split(lixels_df, lixels_df$density_category)

for (category in names(density_segments_list)) {
  cat("\nDensity category:", category, "\n")
  print(density_segments_list[[category]])
}
```

This data frame let us more clearly to see the density of the specific road accident.

```{r}
tmap_mode("view")
tm_shape(lixels) +
  tm_lines(col = "density", lwd = 4) +  
  tm_shape(max_density_segment) +      
  tm_lines(col = "red", lwd = 30, title.col = "Max Density Segment") +  
  tm_shape(acci) +
  tm_dots(size = 0.0001, col = "black")  
...
If we zoom into the the red point ,we will notice that the accidence happened from the junction between roads.

#### **3.4.4**.1 Visualising the highest density

```{r}
SP_h <- admin_level_2_filtered_32647 %>%
...
```{r}
road_hd <- st_intersection(SP_h,road_i_t)
```
...
```{r}
ggplot(acci, aes(x = incident_datetime)) +
...
```{r}
ggplot(acci, aes(x = year(incident_datetime))) +
...
From the four plots,we found that Before the COVID-19 outbreak, there were many road traffic accidents before 2020,and after recovering in 2022.Also ,most of the car accidence happened in the well-known peek time aound 9 am and 19 pm.

```{r}
accident_counts <- acci %>%
...
```{r}
admin_level_1_filtered_owin <- as.owin(admin_level_1_filtered_32647)
admin_level_1_filtered_owin
```

```{r}
class(admin_level_1_filtered_owin)
```

```{r}
tmap_mode("plot")
tm_shape(admin_level_1_filtered_32647)+
  tm_polygons() +
tm_shape(acci) +
  tm_dots(size = 0.1) +
...
```{r}
acci_month_owin <- acci_month_ppp_jit[admin_level_1_filtered_owin]
...
acci$Time <- difftime(acci$incident_datetime, start, units = "days")
acci$Time <- as.numeric(acci$Time)
...
```{r}
acci_hour_owin <- acci_hour_ppp[admin_level_1_filtered_owin]
...
```{r}
acci_yday_owin <- acci_yday_ppp[admin_level_1_filtered_owin]
...
```{r}
acci_yday_owin2 <- acci_yday_ppp_jit[admin_level_1_filtered_owin]
...
acci_high <- acci %>%
  filter(province_en == "Samut Prakan" & 
         agency == "department of highways" & 
         acc_code != "3788977" &
         (Month_num == 4 | Month_num == 12))  
```

```{r}
road_hd <- st_cast(road_hd, "LINESTRING")
acci_high <- st_cast(acci_high, "POINT")
```

```{r}
acci_high <- acci_high[!duplicated(st_coordinates(acci_high)), ]
```
...
The blue line in the figure is significantly higher than the simulated value, indicating that traffic accident points are clustered in space rather than randomly distributed.
...
```{r}
acci_lp <- acci %>%
  dplyr::filter(province_en == "Bangkok" & Year == 2019 & Month_num == 4 )
```
...
```{r}
library(knitr)
cv_scores_adpt_cont <- bw_tnkde_cv_likelihood_calc(
  ...
  verbose = FALSE,
  check = TRUE)
kable(cv_scores_adpt_cont)

```

According to the “leave one out cross validation” method, the optimal set of bandwidths is 600 metres and 70 days.

```{r}
# choosing sample in times 
sample_time <- seq(0, max(acci_lp$Time), 1)
