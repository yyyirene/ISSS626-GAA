...
In this in-class exercise, you will learn how to build predictive model by using geographical random forest method. By the end of this hands-on exercise, we will acquire the skills of:
...
```{r}
#| eval: false
coords_train <- saveRDS(coords_train, "data/model/coords_train.rds" )
coords_test <- saveRDS(coords_test, "data/model/coords_test.rds" )
```
...
```{r}
#| eval: false
GRF_pred <- saveRDS(gwRF_pred, "data/model/GRF_pred.rds")
```
...
```{r}
#| eval: false
readRDS(test_data_p, "data/model/test_data_p.rds")
```
